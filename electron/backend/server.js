...
/* SERVER CONSTANTS */
export function startServer({ port, csvPath, userDataPath, internalToken, onInvalidate }) {
  const app = express();
  app.use(cors());
  ...
  const USER_DATA_PATH = userDataPath;
  ...
  function getCurrentMonthCSV() {
    const now = new Date();
    const ym = `${now.getFullYear()}_${String(now.getMonth() + 1).padStart(2, "0")}`;
    return path.join(process.env.CSV_PATH, `attendance_${ym}.csv`);
  }

  /* CSV Reader */
  function readCSV() {
    if (!process.env.CSV_PATH || !fs.existsSync(process.env.CSV_PATH)) return [];
    ...
    `);
      ...
      db.transaction(() => {
        for (const r of newRows) {
          if (!r.UserID || !r.Time || !r.Date) continue;
